...
export function AddressSearch({ placeholder, onSelect, value = '' }) {
  ...
  useEffect(() => {
    const search = async () => {
      ...
      setLoading(true);
      try {
        // Sanitização rigorosa da query para evitar o Erro 400
        ...
        const url = `https://photon.komoot.io/api/?${searchParams.toString()}`;
        ...
        console.warn("Aviso na busca (Photon):", err.message);
        // Fallback: Se o Photon falhar, não travamos a UI
        setResults([]);
      } finally {
        setLoading(false);
      }
